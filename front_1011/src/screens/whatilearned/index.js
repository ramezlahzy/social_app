    ...
    return (
        <View>
            ...
            {badgeCount > 0 && (
                ...
                >
                    <Text style={{color: "red", fontSize: 16, fontWeight: '700'}}>{badgeCount}</Text>
                </View>
            )}
        </View>
    );
...
const WhatIlearned = () => {
    const [notification, setNotification] = useState({notification: [], count: 0});
    useEffect(() => {
    ...
    const NotificationScreenComponent = () => {
        return (
            <NotificationScreen notification={notification} setNotification={setNotification}></NotificationScreen>
        )
    }
    return (
