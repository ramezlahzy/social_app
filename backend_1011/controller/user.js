...
const add = (req) => new Promise((resolve, reject) => {
...
const generatePin = (userObj) => new Promise((resolve, reject) => {
    const { phoneNumber } = userObj;
    const pinCode = 333333 || Math.floor(100000 + Math.random() * 900000).toString();
    ...
    db.TempUser.findOne({ where: { phoneNumber } })
        .then((existOne) => {
            if(existOne){
                existOne.destroy()
                .then((success) => {
                    db.TempUser.create({ phoneNumber, token: pinCode, expiresAt: expirationTime })
                    .then((tempUser) => {
                        resolve(tempUser)
                    })
                    .catch((error) => {
      ...
      if (tempUser.expiresAt && currentTimestamp <= tempUser.expiresAt) {
        ...
        if (tempUser.token === pinCode) {
          ...
          const token = jwt.sign(userData, secretKey, {
            expiresIn: '365d' // Token expires in 12 month
          }); // Token expires in 12 month

          if(user) {
            return res.status(200).json({ message: 'Login Success', token , state: 1, user});  //state 1 is sign in and state 0 is sign up
          }
          else  return res.status(200).json({ message: 'PhoneNumber Verification Success', token , state: 0});
        } else {
...
const followUser = async ( req, res ) => {
    try {
        ...
        if (user.friend.includes(followUserID)) {
            await user.update({
                friend: user.friend.filter(id => id != followUserID)
            });
            ...
            await user.update({
                friend: [...user.friend, followUserID]
            });
...
const getMyFollowingAndFollowerCnt = async (req, res) => {
    try {
        ...
        const followerCnt = await db.User.count({
            where: db.Sequelize.literal(`JSON_SEARCH(friend, 'one', ${userId}) IS NOT NULL`),
          });
...
const getMyFriend = async ( req, res ) => {
    try {
        ...
        const userIdArray = (user.friend ? user.friend : []).map((id) => parseInt(id.trim()));
        ...
        res.status(200).json({ users: rows, count, page, pageSize, message: "Search Successfully"});
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Internal server error' });
    }
...
const getMyWhatIlearned = async (req, res) => {
    try {
        ...
        res.status(200).json({ count, page, pageSize, entries: rows, message: "Search Successfully" });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Internal server error' });
    }
...
const getMyFavoriteWhatIlearned = async (req, res) => {
    try {
        ...
        const whatIlearnedIDArray = (user.favoriteWIL ? JSON.parse(user.favoriteWIL) : []).map((id) => parseInt(id.trim()));
        ...
        const searchCondition = db.Sequelize.literal(
            `(authorName LIKE '%${keyword ?? ""}%' or content LIKE '%${keyword ?? ""}%')`
        );
...
const getNotification = async (req, res) => {
    try {
        const userID = req.user.id;
        const count = await db.Notification.count({where : {viewed: false, userID: userID}})
        console.log(count);
...
const setNotificationViewed = async (req, res) => {
    try {
        const userID = req.user.id;
        await db.Notification.update({viewed: true}, {where : {viewed: false, userID: userID}})
        const { count, rows } = await db.Notification.findAndCountAll({
            where: { userID : userID},
            order: [['createdAt', 'DESC']]
        })
        res.status(200).json({ message: "Notifications are viewed", notification: rows });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Internal server error' });
    }
...
const getAgreeUserList = async (req,res) => {
    try {
        ...
        res.status(200).json({userList: rows.map(item =>  {return {id: item.id, name: item.fullName, avatar: item.avatar}})});
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Internal server error' });
    }
